...
class EvaluateLog:
    """
    EvaluateLog processes a CSV log file to calculate and display
    evaluation metrics such as the number of episodes, mean episode return,
    mean episode length, mean episode time, etc.
    ...
    Attributes:
        ...
        mean_episode_time (float): The average time per episode.
    ...
    def __init__(self, csv_file, action_type):
        """
        Initializes the EvaluateLog with the CSV file path and action type.

        Parameters:
            csv_file (str): The path to the CSV file containing the log.
            action_type (str): The type of actions (`high-level` or `continuous`).
        """
        self.num_episodes = None
        self.mean_episode_time = None
    ...
    def process_csv(self):
        ...
        number of episodes, mean episode return, mean episode length, mean episode time, etc.
        """
        df = pd.read_csv(self.csv_file, skiprows=1)
        self.num_episodes = len(df)
        self.mean_episode_return = df['r'].mean()
        self.mean_episode_time = df['t'].mean()
    ...
    def display_results(self):
        ...
        print(f"Mean episode time: {self.mean_episode_time}")
...
if __name__ == "__main__":
    ...
    evaluator = EvaluateLog(path, action_type)
